  ...
  providers: [
    ...
    CredentialsProvider({
      ...
      async authorize(credentials) {
        if (!credentials?.username || !credentials.password) return null;
        const user = await validateWPCredentials(credentials.username, credentials.password);
        if (!user || !user.roles.includes("bluu_affiliate")) return null;
        return {
          id: String(user.id),
          name: user.name,
          email: user.email,
          role: "bluu_affiliate" as UserRole,
          wpUserId: user.id,
          affiliateCode: (user.meta as any)?.bluu_affiliate_code ?? undefined,
          affiliateStatus: (user.meta as any)?.bluu_affiliate_status ?? "active",
        };
      },
    }),

    // ── Client portal magic-link login ──────────────────────────────────────
    // Accepts a one-time token generated by /api/portal/auth/resend-invite
    // or /api/portal/auth/forgot-password. Tokens are stored in WP user meta
    // (portal_magic_token + portal_magic_token_expires) and consumed on use.
    CredentialsProvider({
      id: "magic-link",
      name: "Magic Link",
      credentials: {
      ...
      async authorize(credentials) {
        ...
        try {
          const { verifyAndConsumeMagicToken } = await import("@/lib/magicToken");
          const user = await verifyAndConsumeMagicToken(credentials.email, credentials.token);
          if (!user || !user.roles.includes("bluu_client")) return null;
          return {
            id: String(user.id),
            name: user.name,
            ...
            role: "bluu_client" as UserRole,
            wpUserId: user.id,
            clientId: (user.meta?.bluu_client_post_id as number | undefined) ?? undefined,
          };
        } catch (err) {
          console.error("[magic-link authorize]", err);
          return null;
        }
      },
    }),
  ...
  callbacks: {
    async jwt({ token, user, trigger, session: sessionData }) {
      if (user) {
        ...
        token.affiliateCode   = u.affiliateCode;
        token.affiliateStatus = u.affiliateStatus;
      }
    ...
    async session({ session, token }) {
      if (session.user) {
        ...
        u.affiliateCode   = token.affiliateCode;
        u.affiliateStatus = token.affiliateStatus;
      }
      return session;
    },
  },
