...
export default function ScanTool({ variant }: { variant: "public" | "dashboard" }) {
  ...
  const [unlockedScanId, setUnlockedScanId] = useState<string | null>(null);
  ...
  // tool's own public route doesn't need to double as another landing page
  // for people who already have an account — the scan tool has a first-class
  // home inside the dashboard now. A signed-in visitor to the public variant
  // is sent straight there (carrying over any scan-intent query params).
  useEffect(() => {
    if (variant !== "public" || sessionStatus !== "authenticated") return;
    const url = new URL(window.location.href);
    router.replace(`/dashboard/scan${url.search}`);
  }, [variant, sessionStatus, router]);
  ...
  // On the public variant, a signed-in visitor is redirected away before
  // this ever renders — but avoid a flash of the marketing page while that
  // redirect is in flight.
  if (variant === "public" && sessionStatus === "authenticated") return null;
      ...
      <div className="p-6 bg-white border border-line rounded-site shadow-sm">
        {emailSent ? (
          <div className="text-center py-3">
            ...
            {sessionStatus === "authenticated" && unlockedScanId && (
              <Link
                href={`/dashboard/scans/${unlockedScanId}`}
                className="inline-block mt-2 text-blue font-semibold text-xs hover:underline"
              >
                View in scan history &rarr;
              </Link>
            )}
          </div>
        ) : sessionStatus === "authenticated" ? (
  ...
  if (variant === "dashboard") {
    return (
      <div>
        <h1 className="text-2xl font-extrabold text-ink">Run a new scan</h1>
        ...
        <div className="mt-6 max-w-xl">{scanForm}</div>
        {resultsSection}
      </div>
    );
  }
      ...
      <main className="max-w-site mx-auto px-7 py-16">
        <div className="grid lg:grid-cols-[1.2fr_0.8fr] gap-12 items-center">
          ...
          <div>
            ...
            <div className="mt-6">{scanForm}</div>
